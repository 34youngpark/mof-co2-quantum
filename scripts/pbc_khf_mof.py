...
tag = f"calc/pbc_hf/{name}_{basis}_k112"
...
cell.verbose = 4
cell.output = tag + ".log"
cell.max_memory = int(os.environ.get("PYSCF_MAX_MEMORY", 90000))
cell.build()
...
print(f"HOMO-LUMO gap = {(mf.mo_energy[0][cell.nelectron//2]-mf.mo_energy[0][cell.nelectron//2-1])*27.2114:.2f} eV")
with open("calc/pbc_hf/summary.txt", "a") as f:
    f.write(f"{name}\t{basis}_k112\tnao={cell.nao}\tE={e_hf:.8f}\tconv={mf.converged}\t{dt/60:.1f}min\n")